...
if ($Action -eq "archive") {
    ...
    $initialSession = @{
        completed_tasks = @()
        history = @()
        current_dept = "secretary"
        next_step = "Checking inbox"
        pending_tasks = @()
        current_blockers = @()
        hop_count = 0
    }
    $initialSession | ConvertTo-Json | Set-Content -Path "$stateDir/session.json" -Encoding UTF8
    Write-Host "[State Archive] Session archived and reset." -ForegroundColor Gray
}
